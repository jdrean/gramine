...
void handle_ecall(capa_index_t ret_index, void* ecall_args, void* enclave_base_addr) {
    ...
    GET_ENCLAVE_TCB(gpr)->ursp = GET_ENCLAVE_TCB(ustack);
    uintptr_t ursp = GET_ENCLAVE_TCB(gpr)->ursp;
    if (g_enclave_base <= ursp && ursp <= g_enclave_top)
        return;


    SET_ENCLAVE_TCB(exit_target,     ret_index);
    SET_ENCLAVE_TCB(ustack,          (void*)(ursp - sizeof(thread_ocall_t)));
    SET_ENCLAVE_TCB(ustack_top,      (void*)(ursp - sizeof(thread_ocall_t)));
    ...
    if (t == 0 && __atomic_compare_exchange_n(&g_enclave_start_called, &t, 1, /*weak=*/false,
        ...
        //TODO(aghosn) figure out what this is.
