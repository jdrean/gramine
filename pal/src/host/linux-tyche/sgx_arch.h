...
#define SGX_XFRM_RESERVED                                                                \
    (~(SGX_XFRM_LEGACY | SGX_XFRM_AVX | SGX_XFRM_MPX | SGX_XFRM_AVX512 | SGX_XFRM_PKRU | \
       SGX_XFRM_AMX))
...
static inline int enclu(uint32_t eax, uint64_t rbx, uint64_t rcx, uint64_t rdx) {
    __asm__ volatile("enclu" : "+a"(eax) : "b"(rbx), "c"(rcx), "d"(rdx) : "memory", "cc");
    return (int)eax;
}
